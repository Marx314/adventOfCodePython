...
class Day7:
    ...
    def build_tower(self, tower_data):
        tower = dict()
        for program in tower_data:
            info = program.split()
            name = info[0]
            weight = int(info[1].replace('(', '').replace(')', ''))
            ...
            if len(info) > 2:
                ...
                tower[name]['discs'] = discs
        disc = list(itertools.chain(*[tower[program]['discs'] for program in tower]))
        parent = [program for program in tower if program not in disc][0]

        def generate_structure(tower, name):
            total_weight = tower[name]['weight']
            for leaf in tower[name]['discs']:
                if 'childs' not in tower[name]:
                    tower[name]['childs'] = dict()
                tower[name]['childs'][leaf] = tower[leaf]
                total_weight += generate_structure(tower, leaf)
            tower[name]['total_weight'] = total_weight
            del tower[name]['discs']
            return total_weight

        generate_structure(tower, parent)
        for name in list(tower.keys()):
...
cntj (57)'''
    ...
    def puzzle(self):
        return '''suvtxzq (242) -> tdoxrnb, oanxgk
smjsfux (7)
oanxgk (68)
tvvfszm (66)
