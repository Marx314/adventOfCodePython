...
class Day4:
    @split_data
    def count_is_valid(self, passphrases):
        valid = {passphrase: self.is_valid(passphrase) for passphrase in passphrases}
        return len(list(filter(lambda x: x is True, valid.values())))

    @split_data
    def count_is_valid_anagram(self, passphrases):
        valid = {passphrase: self.is_valid_no_anagram(passphrase) for passphrase in passphrases}
        return len(list(filter(lambda x: x is True, valid.values())))
    ...
    def is_valid_no_anagram(self, passphrase):
        sorted_words = [''.join(sorted(word)) for word in passphrase.split()]
...
class Day4Test(TestCase):
    ...
    def test_simple_valid(self):
        passphrase = 'aa bb cc dd'
        self.assertTrue(self.day.is_valid(passphrase))

    def test_simple_valid_words(self):
        passphrase = 'aa bb cc dd aaa'
        self.assertTrue(self.day.is_valid(passphrase))

    def test_simple_invalid(self):
        passphrase = 'aa bb cc dd aa'
        self.assertFalse(self.day.is_valid(passphrase))

    def test_count(self):
        passphrases = '''aa bb cc dd
aa bb cc dd aaa
aa bb cc dd aa'''
        self.assertEqual(self.day.count_is_valid(passphrases), 2)

    def test_count_2(self):
        self.assertTrue(self.day.is_valid_no_anagram('abcde fghij'))
        self.assertFalse(self.day.is_valid_no_anagram('abcde xyz ecdab'))
        self.assertTrue(self.day.is_valid_no_anagram('a ab abc abd abf abj'))
        self.assertTrue(self.day.is_valid_no_anagram('iiii oiii ooii oooi oooo'))
        self.assertFalse(self.day.is_valid_no_anagram('oiii ioii iioi iiio'))
...
oiii ioii iioi iiio'''
        self.assertEqual(self.day.count_is_valid_anagram(passphrases), 3)

    def test_puzzle(self):
        self.assertEqual(self.day.count_is_valid(self.puzzle()), 325)
        self.assertEqual(self.day.count_is_valid_anagram(self.puzzle()), 119)

    def puzzle(self):
        return '''nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
