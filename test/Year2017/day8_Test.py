...
def apply_operation(operation, key, number, register):
    if operation == 'inc':
        register[key] += number
    elif operation == 'dec':
        register[key] -= number
...
@split_data
def max_register(program):
    max_seen = 0
    register = {}
    for instruction in program:
        key, operation, number, _, key_condition, condition, right_operand = instruction.split()
        right_operand = int(right_operand)
        number = int(number)

        if key not in register:
            register[key] = 0
        if key_condition not in register:
            register[key_condition] = 0
        if any([condition == '<=' and register[key_condition] <= right_operand,
                condition == '<' and register[key_condition] < right_operand,
                condition == '==' and register[key_condition] == right_operand,
                condition == '>=' and register[key_condition] >= right_operand,
                condition == '>' and register[key_condition] > right_operand,
                condition == '!=' and register[key_condition] != right_operand]):
            apply_operation(operation, key, number, register)
            if max(register.values()) > max_seen:
                max_seen = max(register.values())

    return max_seen, max(register.values())
...
c inc -20 if c == 10'''
    ...
    def test_simple_part1(self):
        max_seen, max_final = max_register(self.simple_program())
        self.assertEqual(max_final, 1)
        self.assertEqual(max_seen, 10)

    def test_puzzle(self):
        max_seen, max_final = max_register(self.puzzle())
        self.assertEqual(max_final, 3745)
        self.assertEqual(max_seen, 4644)
