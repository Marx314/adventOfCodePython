from src.Year2016.Day4 import Day4
...
class Day4Test(TestCase):
    ...
    def test_aaaa_is_real(self):
        valid = self.day.is_room_valid('aaaaa-bbb-z-y-x-123[abxyz]')
        self.assertTrue(valid)

    def test_a_b_c_d_is_real(self):
        valid = self.day.is_room_valid('a-b-c-d-e-f-g-h-987[abcde]')
        self.assertTrue(valid)

    def test_not_real_is_real(self):
        valid = self.day.is_room_valid('not-a-real-room-404[oarel]')
        self.assertTrue(valid)

    def test_decoy_is_false(self):
        valid = self.day.is_room_valid('totally-real-room-200[decoy]')
...
totally-real-room-200[decoy]'''
        amount = self.day.sum_valid(rooms)
        self.assertEqual(1514, amount)

    def test_puzzle_one(self):
        amount = self.day.sum_valid(self.puzzle())
        self.assertEqual(158835, amount)

    def test_sample_part2(self):
        result = self.day.search('qzmt-zixmtkozy-ivhz-343', search='very encrypted name')
        self.assertEqual(result, 343)
        result = self.day.search('nvrgfezqvu-avccpsvre-crsfirkfip-217[nrzcd]', search='jellybean')
        self.assertEqual(result, 217)

    def test_puzzle_part2(self):
        result = self.day.search(self.puzzle(), search='north')
        self.assertEqual(result, 993)

    def puzzle(self):
        return '''nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
