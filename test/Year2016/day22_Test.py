...
from src.Year2016.Day22 import Day22
...
class Day22Test(TestCase):
    ...
    def test_map_well_build(self):
        result = self.day.build_map(self.puzzle())
        self.assertEqual(result[0][0], {'Used': 65, 'Avail': 20, 'x': 0, 'y': 0})
        self.assertEqual(result[36][27], {'Used': 67, 'Avail': 23, 'x': 36, 'y': 27})

    def test_count_viable_pairs(self):
        result = self.day.count_pairs(self.puzzle())
        self.assertEqual(result, 1007)

    def test_sample_part2(self):
        path_length = self.day.shortest_path(self.simple_puzzle())
        self.assertEqual(path_length, 7)

    def test_puzzle_part2_path_a_wall(self):
        length = self.day.shortest_path_to_wall(self.puzzle())
        self.assertEqual(length, 35)

    def test_puzzle_part2_path_complete(self):
        path_length = self.day.shortest_path(self.puzzle())
        self.assertEqual(path_length, 242)
