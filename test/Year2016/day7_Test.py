...
from src.Year2016.Day7 import Day7
...
class Day7Test(TestCase):
    ...
    def test_simple_data(self):
        result = self.day.valid_tls('abba[mnop]qrst')
        self.assertTrue(result)
        result = self.day.valid_tls('abcd[bddb]xyyx')
        self.assertFalse(result)
        result = self.day.valid_tls('aaaa[qwer]tyui')
        self.assertFalse(result)
        result = self.day.valid_tls('ioxxoj[asdfgh]zxcvbn')
        self.assertTrue(result)

    def test_multiple_hypernets(self):
        result = self.day.valid_tls('xxxxxxxxx[rskpebsqdfctoqg]hbwageeiufvcmuk[wfvdhxyzmfgmcphpfnc]abba')
        self.assertTrue(result)

    def test_edgecase_for_regex(self):
        valid = self.day.valid_tls('zjjzriicccctoem[zvfukltptxfcxazyi]ybrstbqfckhzpyin')
        self.assertTrue(valid)

    def test_puzzle(self):
        count_valid = self.day.count_valid_tls(self.puzzle())
        self.assertEqual(115, count_valid)

    def test_valid_ssl(self):
        self.assertTrue(self.day.valid_ssl('aba[bab]xyz'))
        self.assertFalse(self.day.valid_ssl('xyx[xyx]xyx'))
        self.assertTrue(self.day.valid_ssl('aaa[kek]eke'))
        self.assertTrue(self.day.valid_ssl('zazbz[bzb]cdb'))

    def test_valid_ssl_puzzle(self):
        count_valid = self.day.count_valid_ssl(self.puzzle())
        self.assertEqual(231, count_valid)
